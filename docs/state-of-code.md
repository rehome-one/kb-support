...
|---|---|---|---|
| ~~TD-OAS-001~~ | ~~handoff `04_openapi.yaml` (v1.1, из ТЗ Приложение A) содержит legacy `nullable: true` (deprecated в OpenAPI 3.1). Redocly strict lint падает с 28 ошибками.~~ **Closed:** PR #11 — сгенерирован production `docs/openapi.yaml` (`scripts/generate_production_openapi.py`: `nullable: true` → 3.1 `type: [<T>, "null"]`); `redocly lint` = **0 errors**. Handoff остаётся immutable (CI advisory), production — strict blocking. | ~~P3~~ closed | Контрактные тесты (#4) и клиенты используют `docs/openapi.yaml`. Дельты реализации (`/history` и пр.) — follow-up к спека-полноте (#4 / отдельная задача) |
| ~~TD-RBT-001~~ | ~~Архитектурная константа держится на дисциплине агентов + Reviewer чек-листе.~~ **Closed:** PR #3 (AT-001 CI script). Теперь enforce'ится через `scripts/check-arch-constraint.sh` в CI job `arch-constraint`. | ~~P2~~ closed | Reviewer полагается на CI; дополнительно может greпать для подозрений на обход через `__import__` | 
...
### ~~AT-001 — Проверка архитектурной константы в CI~~ ✅ Closed (PR #3)

Реализовано в `scripts/check-arch-constraint.sh` + CI job `arch-constraint`
+ 4 self-test фикстуры в `tests/arch-constraint/`. Запрещены:
- Python imports `from (rehome_kb_platform|kb_platform|kb_search|...)`.
- TypeScript imports `from "(rehome-kb-platform|kb-platform|...)"`.
- SQL `(FROM|JOIN|UPDATE|INTO|TABLE) (users|premises|bookings|collaborators|...)`.

Allowlist: inline-комментарий `# arch-allow: <обоснование ≥10 chars>`.
Локально: `make arch-check` из `backend/`. **TD-RBT-001 closed.**
