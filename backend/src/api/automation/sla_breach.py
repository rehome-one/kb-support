...
применяет on_sla_breach-правила: `escalate` идемпотентен как переход, но
`set_priority`/`notify` повторятся каждый цикл. До ops-воркера спама нет (actor
инертен). Дедуп-маркер — follow-up #120.
